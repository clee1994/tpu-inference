...
def sc_gather_reduce(
  ...
  """
    ...
    with mlir.make_ir_context() as ctx, ir.Location.unknown():
        ...
        memory_space_semaphore = ir.Attribute.parse(
        ...
        enable_all_sublanes_mask = ir.DenseBoolArrayAttr.get([True] *
                                                             vreg_size)
            ...
            def const_lut(val, ty=None):
                ...
                if (val, ty) not in constants:
                    with ir.InsertionPoint.at_block_begin(func_op.entry_block):
                        constants[(val, ty)] = arith.constant(
            ...
            def fill_load_offset_tile(offset_tile_local, idx_tile_local,
                ...
                iota = arith.constant(
                    ...
                    ir.DenseIntElementsAttr.get(
                        array.array("i", [
                            i * mul_mem_layout for i in range(row_chunk_size)
                        ]),
                ...
                loop_over_idx.attributes[
                    "sc.loop_unroll_factor"] = ir.IntegerAttr.get(
                        i32, loop_unroll_factor_2)
                if loop_parallel_access_2:
                    loop_over_idx.attributes[
                        "sc.parallel_access"] = ir.UnitAttr.get()
                with ir.InsertionPoint(loop_over_idx.body):
                    i = loop_over_idx.induction_variable
                    base_idx = vector.extract(
                        idx_loaded,
                        dynamic_position=[i],
                        static_position=ir.DenseI64ArrayAttr.get(
                            [-9223372036854775808]),
                    )
                    ...
                    if is_bf16:
                        base_idx_rem = arith.divui(
                            arith.remui(base_idx, const_lut(8, i32)),
                            const_lut(2, i32))
                    else:
                    ...
                    start_vec = arith.addi(
                        vector.broadcast(_I32[row_chunk_size], base_idx), iota)
                    loop_j = scf.ForOp(
                        const_lut(0),
                    ...
                    add_vec = vector.broadcast(
                        _I32[vreg_size],
                        const_lut(int(mul_mem_layout * rem_vreg * vreg_size),
                                  i32),
                    )  # 4 comes from bf16
        ...
        """
                lin_idx = arith.divui(
                    lin_idx, const_lut(16))  # reading in increments of 16

                # parity
                # (i >> 3) & 1
                parity = arith.andi(arith.shrui(lin_idx, const_lut(3)),
                                    const_lut(1))
                ...
                lin_idx = arith.ori(
                    ...
                    arith.shli(arith.shrui(lin_idx, const_lut(4)),
                               const_lut(3)),
                )
        ...
        """
                ...
                if weights_local is not None:
                    ...
                    raw_weights_f32 = arith.select(is_odd, weights_odds,
                                                   weights_evens)
                ...
                loop_pack_scratch = scf.ForOp(const_lut(0),
                                              const_lut(col_chunk_size),
                                              const_lut(vreg_size))
                loop_pack_scratch.attributes[
                    "sc.loop_unroll_factor"] = ir.IntegerAttr.get(
                        i32, loop_unroll_factor_1)
                ...
                with ir.InsertionPoint(loop_pack_scratch.body):
                    ...
                    def get_row_val(row_idx):
                        if is_bf16:
                            ...
                            is_odd_scalar = arith.cmpi(arith.CmpIPredicate.eq,
                                                       parity_of_row,
                                                       const_lut(0, i32))
                            return arith.select(is_odd_scalar, vec_f32_odds,
                                                vec_f32_evens)
                        else:
                            return tpu.load(
                                _F32[vreg_size],
            ...
            def fill_out_offset_tile(offset_tile_out_local,
                                     col_pos,
                                     row_pos=None):
        ...
        """
                if row_pos is not None:
                    ...
                    row_vec_offset = vector.broadcast(_I32[row_chunk_size],
                                                      row_vec_offset)
                ...
                iota = arith.constant(
                    ...
                    ir.DenseIntElementsAttr.get(
                        array.array("i",
                                    [i * 4 for i in range(row_chunk_size)
                                     ]),  # 4 is key here for bf16
                        type=_I32[row_chunk_size],
                    ),
                )
                ...
                with ir.InsertionPoint(loop_over_idx.body):
                    ...
                    start_vec = arith.addi(
                        vector.broadcast(_I32[row_chunk_size],
                                         arith.index_cast(i32, loop_i)),
                        iota,
                    )
                    loop_j = scf.ForOp(
                        const_lut(0),
                    ...
                    with ir.InsertionPoint(loop_j.body):
                        ...
                        add_vec = vector.broadcast(
                            _I32[vreg_size], const_lut(4 * vreg_size, i32))
                ...
                last_full_vec = tpu.load(
                    ...
                    offset_tile_out_local,
                    [
                        const_lut(offset_sizes_out - int(vreg_size *
                                                         (1 + rem_vreg)))
                    ],
                    enable_all_sublanes_mask,
                )
                add_vec = vector.broadcast(_I32[vreg_size],
                                           const_lut(
                                               int(4 * rem_vreg * vreg_size),
                                               i32))  # 4 comes from bf16
            ...
            offset_sizes_out = ((row_chunk_size // reduce_group_size) *
                                (col_chunk_size // 128) // 2
            ...
            sflag_0 = tpu.sem_alloc(
                ir.MemRefType.get((),
                                  dma_semaphore_type,
                                  memory_space=memory_space_semaphore))
            sflag_1 = tpu.sem_alloc(
                ir.MemRefType.get((),
                                  dma_semaphore_type,
                                  memory_space=memory_space_semaphore))
            sflag_out_0 = tpu.sem_alloc(
                ir.MemRefType.get((),
                                  dma_semaphore_type,
                                  memory_space=memory_space_semaphore))
            sflag_out_1 = tpu.sem_alloc(
                ir.MemRefType.get((),
                                  dma_semaphore_type,
                                  memory_space=memory_space_semaphore))
            ...
            with ir.InsertionPoint(loop_col_chunk.body):
                ...
                offset_tile_out_0 = fill_out_offset_tile(
                    offset_tile_out_0, col_chunk_ij)
                ...
                base_idx_val = arith.index_cast(
                    i32, arith.addi(global_row_idx_start, loop_row_chunk_idx))
                base_idx_val = tpu.assume_multiple(base_idx_val, 8)
                source_slice = tpu.memref_slice(
                    ir.MemRefType.get((row_chunk_size, ),
                                      i32,
                                      memory_space=memoryspace_hbm),
                    idx_ref,
                ...
                tpu.wait_dma2(semaphore=sflag_0,
                              src=source_slice,
                              dst=idx_tile_0)

                if topk_weights is not None:
                    # The linear index is global_row_idx_start + loop_row_chunk_idx
                    lin_idx = arith.addi(global_row_idx_start,
                                         loop_row_chunk_idx)
                    parity_0 = load_weights(lin_idx, weights_tile_0,
                                            sflag_weights_0)
                ...
                offset_tile_0, idx_parity_0 = fill_load_offset_tile(
                    offset_tile_0, idx_tile_0, col_chunk_ij)
                ...
                loop_over_row_chunks = scf.ForOp(
                    ...
                    upper_bound=const_lut(
                        row_chunk_size *
                        ((idx.shape[0] // num_sc // row_chunk_size) - 1)),
                    ...
                    iter_args=[
                        ...
                        offset_tile_out_1,
                        weights_tile_0
                        if topk_weights is not None else scratch_0,
                        weights_tile_1
                        if topk_weights is not None else scratch_0,
                        sflag_weights_0
                        if topk_weights is not None else sflag_0,
                        sflag_weights_1
                        if topk_weights is not None else sflag_0,
                ...
                with ir.InsertionPoint(loop_over_row_chunks.body):
                    ...
                    tpu.wait_dma2(semaphore=sflag_1,
                                  src=source_slice,
                                  dst=idx_tile_1)
                    ...
                    if topk_weights is not None:
                        ...
                        lin_idx = arith.addi(global_row_idx_start,
                                             loop_row_chunk_idx)
                        parity_1 = load_weights(lin_idx, weights_tile_1,
                                                sflag_weights_1)

                    offset_tile_1, idx_parity_1 = fill_load_offset_tile(
                        offset_tile_1, idx_tile_1, col_chunk_ij)
                    ...
                    # wait stream #0
                    tpu.wait_indirect_dma(semaphore=sflag_0,
                                          src=op_ref,
                                          dst=scratch_0)
                    ...
                    offset_tile_out_0 = fill_out_offset_tile(
                        ...
                        arith.divui(
                            arith.addi(
                                global_row_idx_start,
                                arith.subi(loop_row_chunk_idx,
                                           const_lut(row_chunk_size)),
                            ),
                    ...
                    # wait stream #1
                    tpu.wait_indirect_dma(semaphore=sflag_1,
                                          src=op_ref,
                                          dst=scratch_1)
                    ...
                    offset_tile_out_1 = fill_out_offset_tile(
                        ...
                        col_chunk_ij,
                        arith.divui(
                            arith.addi(global_row_idx_start,
                                       loop_row_chunk_idx),
                            const_lut(reduce_group_size),
                        ),
                    )
                    ...
                    # wait stream #0 out
                    tpu.wait_indirect_dma(semaphore=sflag_out_0,
                                          src=scratch_out_0,
                                          dst=out_ref)
                    # wait stream #1 out
                    tpu.wait_indirect_dma(semaphore=sflag_out_1,
                                          src=scratch_out_1,
                                          dst=out_ref)
                ...
                epi_idx_loc = row_chunk_size * (
                    (idx.shape[0] // num_sc // row_chunk_size) - 1)
                add_f = arith.divui(global_row_idx_start,
                                    const_lut(reduce_group_size))

                # setup stream for #1
                base_idx_val = arith.index_cast(
                    i32,
                    arith.addi(global_row_idx_start, const_lut(epi_idx_loc)))
                base_idx_val = tpu.assume_multiple(base_idx_val, 8)
                source_slice = tpu.memref_slice(
                    ir.MemRefType.get((row_chunk_size, ),
                                      i32,
        ...
        window_params = [
            ir.DictAttr.get({
                "transform_indices":
                ir.Attribute.parse("affine_map<(n,m) -> (0)>"),
            }),
            ir.DictAttr.get({
                "transform_indices":
                ir.Attribute.parse("affine_map<(n,m) -> (0,0)>"),
            }),
            ir.DictAttr.get({
                "transform_indices":
                ir.Attribute.parse("affine_map<(n,m) -> (0,0)>"),
        ...
        f.attributes["tpu.core_type"] = ir.Attribute.parse(
            "#tpu.core_type<sc_vector_subcore>")
        assert f.verify(), f
        m = ir.Module.create()
        m.body.append(f)
